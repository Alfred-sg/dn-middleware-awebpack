...
module.exports = function (options) {
  ...
  return async function (next) {
    ...
    if (this.emit) this.emit('webpack.opts', options, webpack);

    const { 
      name, location,
      configFile = './webpack.config.js', configObject, 
      watchOpts = { aggregateTimeout: 600, ignored: /node_modules/ }, 
      ...opts 
    } = options;

    let config = configObject || await generateConfig(this, opts);

    //config
    let customConfigFile = path.resolve(this.cwd, configFile);
    ...
    if (this.emit) this.emit('webpack.config', config, webpack, options);
    ...
    if (this.emit) this.emit('webpack.compiler', compiler, webpack, options);

    if (opts.watch) {
      if (this.emit) this.emit('webpack.watch', compiler, webpack, options);
      ...
      if (this.emit) this.emit('webpack.run', compiler, webpack, options);
